...
class GastoController:
    ...
    async def get_by_id(self, gasto_id: int, user: dict = Depends(get_current_user_with_roles(allowed_roles=["Operador", "Admin"]))):
        gasto = await self.service.get_by_id(gasto_id)
    ...
    async def update(self, gasto_id: int, gasto: GastoUpdate, user: dict = Depends(get_current_user_with_roles(allowed_roles=["Admin"]))):
        await self.service.update(gasto_id, gasto)
        return ResponseSchema(detail="Gasto actualizado con éxito")

    async def delete(self, gasto_id: int, user: dict = Depends(get_current_user_with_roles(allowed_roles=["Admin"]))):
        await self.service.delete(gasto_id)

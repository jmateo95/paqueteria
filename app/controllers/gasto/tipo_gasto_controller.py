...
class TipoGastoController:
    ...
    async def get_by_id(self, tipo_gasto_id: int, user: dict = Depends(get_current_user_with_roles(allowed_roles=["Operador", "Admin"]))):
        tipo_gasto = await self.service.get_by_id(tipo_gasto_id)
    ...
    async def update(self, tipo_gasto_id: int, tipo_gasto: TipoGastoUpdate, user: dict = Depends(get_current_user_with_roles(allowed_roles=["Admin"]))):
        await self.service.update(tipo_gasto_id, tipo_gasto)
        return ResponseSchema(detail="Tipo de gasto actualizado con éxito")

    async def delete(self, tipo_gasto_id: int, user: dict = Depends(get_current_user_with_roles(allowed_roles=["Admin"]))):
        await self.service.delete(tipo_gasto_id)

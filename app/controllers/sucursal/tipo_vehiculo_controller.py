...
class TipoVehiculoController:
    ...
    async def create(self, tipo_vehiculo: TipoVehiculoCreate, user: dict = Depends(get_current_user_with_roles(allowed_roles=["Admin"]))):
        tipo_vehiculo = await self.service.create(tipo_vehiculo)
        return ResponseSchema(detail="Tipo de vehículo creado con éxito", result=tipo_vehiculo)
    ...
    async def update(self, id: int, tipo_vehiculo: TipoVehiculoUpdate, user: dict = Depends(get_current_user_with_roles(allowed_roles=["Admin"]))):
        tipo_vehiculo = await self.service.update(id, tipo_vehiculo)
        return ResponseSchema(detail="Tipo de vehículo actualizado con éxito", result=tipo_vehiculo)
